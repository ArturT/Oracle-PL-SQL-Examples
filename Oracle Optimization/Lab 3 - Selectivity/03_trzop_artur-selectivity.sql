...
set serveroutput on;
set feedback on;
set timing on; -- włącz wyświetlanie czasu wykonania zapytania
...
BEGIN
	...
	-- generowanie mieszkańców Krakowa odbędzie się tylko gdy jeszcze tego wcześniej nie robiono
	...
	IF int1 != 20 THEN
		...
		FOR i IN 1..10000 LOOP
			liczba := to_number(TRUNC(dbms_random.value(1,1000000))); -- losujemy jedną z 1mln osob ktora bedzie przypisana do ktoregos urzedu pocztowego w krakowie
...
/*
	...
	###	ALL_ROWS - z preferencją sort-merge lub hash-join. Jeśli nie ma statystyk dla tabeli to użycie RBO
	...
	### str.23. Określa liczbę bajtów przydzieloną dla obszaru sortowania w ramach jednej sesji użytkownika.
...
IS
	...
	BEGIN
		...
		--### Występują dwa warunki na imie i plec!!
		...
		--### Selektywnośc całkowita powinna być sumą selektywności cząstkowych dla każdej unikatowej wartości kolumny.
		...
		--### Bardzo duża selektywnosc 0.0067% ###################################	
		...
		--### Obliczamy selektywnosc dla złączonych tabel
		...
		--### Bardzo mała selektywnosc na poziomie 50% ###################################	
		--### Pobrac wszystkie kobiety z Krakowa
		...
		czas := PACKAGE_SELEKTYWNOSC.P_LICZ_CZAS_START(NULL); -- startujemy licznik czasu
		--### warunek: Pobierz kobiety o imieniu na litere S i urodzone po roku 1980 włącznie
...
--### Na kolumne płeć powinien byc nałożony index bitmapowy ponieważ ma ona niską kardynalnosc
-- indexy BITMAP nie działają w wersji darmowej bazy (ORA-00439: feature not enabled: Bit-mapped indexes)
/*
