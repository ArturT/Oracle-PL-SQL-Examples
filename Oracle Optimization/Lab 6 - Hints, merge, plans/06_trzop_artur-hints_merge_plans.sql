...
-- Wyœwietla rozbudowany plan
SELECT plan_table_output FROM table(dbms_xplan.display(NULL,'PLAN_1_A','TYPICAL'));
/*
	### Zostanie uzyty index IX_OSO_PLECIMIEDATAURODZ na³o¿ony na trzy kolumny wystêpuj¹ce w where
...
--### Podzapytania
-- ### Zapytanie pobiera osoby ktore urodzily sie w latach +/- 5 lat od œredniej daty urodzenia wszystkich osob
...
/*
	...
	### ORDERED - wymusi z³¹czenie tabel wg. kolejnosci wyst¹pienia po FROM. Tabele zostan¹ po³¹czone wg. kolejnosci OSOBY do ADRES_POCZTY
				Gdy t¹ opcje wy³¹czymy to ³¹czona jest ADRES_POCZTY do OSOBY poniewaz tabela ADRES_POCZTY ma mniej wierszy
*/
...
-- Za pomoc¹ jednego polecenia dokonujemy aktualizacji lub dodania wartoœci w tabeli
PROMPT ######################################################
PROMPT ###
