...
--# Włączamy polskie znaki w konsoli
...
-- Dla płci mozemy wszystkie dane przeanalizowac poniewaz plec ma niską kardynalnosc
...
--# Analizujemy cały index poniewaz tabela ADRES_POCZTY jest mniejsza
...
BEGIN
	...
	-- można też użyć estimate_percent=>DBMS_STATS.AUTO_SAMPLE_SIZE
	DBMS_STATS.GATHER_TABLE_STATS(
		ownname => 'Trzop_Artur', 
		tabname => 'OSOBY', 
		estimate_percent => 10,
...
--### Wyświetlenie informacji o zebranych statystykach
...
/*
	...
	ADRES_POCZTY ADR_MIASTO                   1943         254 HEIGHT BALANCED	<=========== Histogram o zrównoważonej wysokości (Dzieli cały zbiór wartości kolumny na przedziały o tej samej liczbie rekordów.)
	...
	OSOBY        OSO_IMIE                      594         254 HEIGHT BALANCED 	<=========== Histogram o zrównoważonej wysokości
	OSOBY        OSO_NAZWISKO                19877           1 NONE
	OSOBY        OSO_PLEC                        2           2 FREQUENCY		<=========== Histogram częstotliwości (Każda unikatowa wartość kolumny tworzy jeden przedział, który zawiera liczbę wystąpień tej wartości)
...
--###<<< Dla zapytania: Pobierz kobiety o imieniu na litere S i urodzone po roku 1980 włącznie >>>
PROMPT #####################################################################
PROMPT ### 1. Dla zapytania: Pobierz kobiety o imieniu na litere S i urodzone po roku 1980 włącznie
...
/*
-- Wyświetlenie planu inną metodą
...
--### Wyświetlamy dostępne histogramy
PROMPT
PROMPT
PROMPT #####################################################################
...
/*
--### Inna metoda wyświetlania w konsoli histogramu
...
-- ENDPOINT_ACTUAL_VALUE Wyświetli końce przedziałów
...
/* ############# Wyłączone bo zajmuje dużą część ekranu <------------------<<<<<<<<<<<<================
PROMPT @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
...
-- Można też uzyc wyswietlania +++ zamiast liczby 3 itd, ale w konsoli jest to nieczytelne
...
/*
	Wskazówka: Na serwerze PK nie ma dostępu do tabel: DBA_TAB_STATISTICS, DBA_IND_STATISTICS
*/

--## Pobranie informacji o ilosci wierszy w tabeli oraz ilości przeanalizowanych wierszy a takze czasie ostatniej analizy 
